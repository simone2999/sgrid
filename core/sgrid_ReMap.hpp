...
public:
    ...
    void from_pgrid_to_pblock(Field& parallel, Field& serial, int tile_number)
    {
        ...
        if (profile_)
            MPI_Barrier(parallel.grid()->raw_comm());
        ...
        if constexpr (Dim == 3) {
            sgrid::parallel_for(
                ...
                SGRID_LAMBDA(int i, int j, int k) {
                    ...
                    int node_offset = (i - grid_host.margin[0]) * (grid_host.dim[1] * grid_host.dim[2]) + (j - grid_host.margin[1]) * grid_host.dim[2] + (k - grid_host.margin[2]);
            ...
            if constexpr (Dim == 3) {
                ...
                sgrid::parallel_for(
                    "ReMap::p_block_pack_data::CopyDataToSlicedBuffer", range, SGRID_LAMBDA(int i, int j, int k) {
                        ...
                        for (int l = 0; l < tile_size; ++l) {
                            auto value = b[l];
                            p_block_buffer[proc_offset + (i * dims[1] * dims[2] + j * dims[2] + k) * tile_size + l] = value;
                        }
                    });
    ...
    {
        assert((parallel_field.block_size() / serial_field.block_size()) * serial_field.block_size() == parallel_field.block_size());
