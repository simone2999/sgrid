...
namespace sgrid {
    ...
    public:
        ...
        void p_grid_pack_data(Field& parallel_field, int tile_size, int tile_number) {
            ...
            int num_partitions = num_partitions_;
            int block_parition_per_rank = block_size / num_partitions;

            if (num_partitions < comm_size) {
                assert(tile_number == 0);
                if (tile_number > 0) {
                    if (!pgrid->comm_rank()) {
                        printf(
                            "ERROR(sgrid): for number of partitions that are smaller than the size of the "
                            "communicator, only "
                            "1 tile is allowed\n");
                    }

                    MPI_Abort(pgrid->raw_comm(), -1);
                }
            }

            if constexpr (Dim == 3) {
                sgrid::parallel_for(
                    "ReMap::pack_data::CopyBlockSliceToBuffer", pgrid->md_range(), SGRID_LAMBDA(int i, int j, int k) {
                        ...
                        for (int r = 0; r < num_partitions; ++r) {
                            int block_start = (block_parition_per_rank * r) + (tile_number * tile_size);

                            for (int l = 0; l < tile_size; ++l) {
                                auto value = b[block_start + l];
        ...
        void p_grid_unpack_data(Field& parallel_field, int tile_size, int tile_number) {
            ...
            if constexpr (Dim == 3) {
                sgrid::parallel_for(
                    "ReMap::pack_data::CopyBlockSliceFromBuffer", pgrid->md_range(), SGRID_LAMBDA(int i, int j, int k) {
                        auto b = field_host.block(i, j, k);
        ...
        void p_block_pack_data(Grid& parallel_grid, Field& serial_field) {
            ...
            // If we do not have enough blocks ignore
            if (parallel_grid.comm_rank() >= num_partitions_) {
                return;
            }
        ...
        void init(Field& parallel_field, Field& serial_field, int num_partitions = -1) {
            assert((parallel_field.block_size() / serial_field.block_size()) * serial_field.block_size() ==
                   parallel_field.block_size());

            if (num_partitions == -1) {
                num_partitions = parallel_field.grid()->comm_size();

                if (num_partitions * serial_field.block_size() > parallel_field.block_size()) {
                    num_partitions = parallel_field.block_size() / serial_field.block_size();
                }
            }
