...
namespace sgrid {
    ...
    public:
        ...
        bool init(int dim) {
            ...
            if (stride == 1) {
                CATCH_MPI_ERROR(MPI_Type_contiguous(count * block_size, real_type, &recv_type_));

                CATCH_MPI_ERROR(MPI_Type_contiguous(count * block_size, real_type, &send_type_));
            } else {
                CATCH_MPI_ERROR(MPI_Type_vector(count, block_size, stride * block_size, real_type, &recv_type_));

                CATCH_MPI_ERROR(MPI_Type_vector(count, block_size, stride * block_size, real_type, &send_type_));
            }
            ...
            if (Grid::Dim == 3) {
                for (int disp_num_0 = 0; disp_num_0 < 2; ++disp_num_0) {
                    for (int disp_num_1 = 0; disp_num_1 < 2; ++disp_num_1) {
                        ...
                        send_idx[d0] =
                            (disp[d0][disp_num_0] > 0) ? (g_host.dim[d0] - 1 + g_host.margin[d0]) : g_host.margin[d0];

                        recv_idx[d0] = (disp[d0][disp_num_0] > 0) ? (g_host.dim[d0] - 1 + 2 * g_host.margin[d0]) : 0;

                        //////////////////////////////////////////////////////////////////////

                        send_idx[d1] =
                            (disp[d1][disp_num_1] > 0) ? (g_host.dim[d1] - 1 + g_host.margin[d1]) : g_host.margin[d1];

                        recv_idx[d1] = (disp[d1][disp_num_1] > 0) ? (g_host.dim[d1] - 1 + 2 * g_host.margin[d1]) : 0;
    ...
    public:
        ...
        bool init(int dim) {
            ...
            for (int s = 0; s < n_sides; ++s) {
                if (neigh_rank[s] == MPI_PROC_NULL) continue;
                ...
                CATCH_MPI_ERROR(MPI_Type_create_subarray(tensor_dir,
                                                         array_of_sizes,
                                                         array_of_subsizes,
                                                         array_of_starts,
                                                         MPI_ORDER_C,
        ...
        void exchange() override {
            ...
            for (int s = 0; s < 2; ++s) {
                ...
                CATCH_MPI_ERROR(MPI_Sendrecv(field_host.ptr(),
                                             1,
                                             send_type[s],
                                             neigh_rank[s],
                                             tag,
