...
// Analytic function
SGRID_INLINE_FUNCTION Real fun(const Real x, const Real y) { return (x * (1 - x) * y * (1 - y)) * (x + y); }
...

    {
        ...
        sgrid::parallel_for(
            "Index", g->md_range(), SGRID_LAMBDA(int i, int j) {
                ...
                solution_dev.ref(i, j) = (g_dev.start[0] + i - g_dev.margin[0]) * g_dev.global_dim[1] +
        ...
        sgrid::parallel_for(
            "BC", g->md_range_with_ghosts(), SGRID_LAMBDA(int i, int j) {
                ...
                bool is_boundary = x_id == 0 || x_id == (nx + 2) || y_id == 0 || y_id == (ny + 2);
            ...
            for (int color = 0; color < 2; ++color) {
                ...
                sgrid::parallel_for(
                    "LaplaceOp", g->md_range(), SGRID_LAMBDA(int i, int j) {
                        ...
                        Real acc = (solution_dev.ref(i - 1, j) + solution_dev.ref(i + 1, j)) / (hx * hx) +
                                   (solution_dev.ref(i, j - 1) + solution_dev.ref(i, j + 1)) / (hy * hy);
            ...
            if (iter % check_residual_each == 0 || (iter == (max_iter - 1))) {
                ...
                sgrid::parallel_reduce(
                    ...
                    SGRID_LAMBDA(int i, int j, Real &acc) {
                        ...
                        Real off_diag_x = (solution_dev.ref(i - 1, j) + solution_dev.ref(i + 1, j)) / (hx * hx);

                        Real off_diag_y = (solution_dev.ref(i, j - 1) + solution_dev.ref(i, j + 1)) / (hy * hy);
                ...
                Real g_norm = 0;
                MPI_Allreduce(&lg_norm, &g_norm, 1, sgrid::MPIType<Real>(), MPI_SUM, g->raw_comm());
