...
function(add_codecov _targetname _testrunner _outputname)
  ...
  add_custom_target(
    ...
    COMMAND ./${_testrunner}
    # Parallel run
    COMMAND ${MPIEXEC_EXECUTABLE} -np ${MPIEXEC_MAX_NUMPROCS} ./${_testrunner}
    COMMAND ${LCOV_EXE} --version
    COMMAND ${GCOV_EXE} --version
    COMMAND ${CMAKE_CXX_COMPILER} --version
    # Capturing lcov counters and generating report
    COMMAND ${LCOV_EXE} --directory . --base-directory . --capture --output-file
            ${coverage_info}
    COMMAND
      ${LCOV_EXE} --remove ${coverage_info} '/usr*' '*/test/*' '*/_deps/*'
      '*/ext/*' '/usr/include/*' '/usr/lib/*' '/Applications/*' '/examples/*'
      '*/include/*' -o ${coverage_info}
  ...
  add_custom_target(
    ${_targetname}_html
    COMMAND ${GENHTML_EXE} --prefix ${CMAKE_SOURCE_DIR} --ignore-errors source
            ${coverage_info} --output-directory ${_outputname}
    WORKING_DIRECTORY ${CMAKE_BINARY_DIR})
  ...
  add_custom_target(
    ${_targetname}_upload
    COMMAND cp ../scripts/codecoverage/codecov.sh ./
